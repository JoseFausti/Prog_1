...
b)_ precio no es un número, por lo que no puede operarse. Debe pasarse a int o float.
...
a= int( (num /10 ) //1 )
b= int(num % 10)
print(f"Su numero invertido es: {b}{a}")
...
hhmmss=str(input("Ingrese el horario de salida en formato HH:MM:SS : "))
hhmmss=hhmmss.split(":")
hh=(hhmmss[0])
mm=(hhmmss[1])
ss=(hhmmss[2])
# Pasamos las variables a numeros enteros
hh=int(hh)
mm=int(mm)
ss=int(ss)
# Preguntamos cuantos segundos demoro en llegar
tiempo_transcurrido=int(input("¿Cuántos segundos demoró en llegar? "))
ss+=tiempo_transcurrido
if ss >=60:
    ss -= 60
    mm += 1
print("Hora de llegada: ",hh,":",mm,":",ss)    
...
# 20)_Hacer otra versión del programa, pero esta vez almacenado todo en una única variable
# con formato DDMMAAAA.
DDMMAAAA=input("Ingrese el dia de su nacimiento en formato DDMMAAAA: ")
dd=DDMMAAAA[0:2]
mm=DDMMAAAA[2:4]
aaaa=DDMMAAAA[4:8]
